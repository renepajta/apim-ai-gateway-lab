# Demo talk track — APIM AI Gateway for your AI platform / GenAI Hub (v3, 25 min)

**Audience:** your AI platform / GenAI Hub team — platform-provider mindset (multi-tenant, multi-region, governance).
...
1. `cd C:\repos\chief-architect-daily\tasks\20260428-apim-ai-gateway-lab\demo`
...
4. Open three browser tabs:
   ...
   - App Insights `appi-lab-aigw-demo` → **Workbooks** → "AI Gateway — per-tenant tokens"
5. Open a **second terminal** for the CB break (Section 2). Have these two commands in your clipboard, ready to paste:
   ...
   az rest --method PATCH `
     --uri "https://management.azure.com/subscriptions/00000000-0000-0000-0000-000000000000/resourceGroups/rg-lab-aigw-demo-swc/providers/Microsoft.ApiManagement/service/apim-lab-aigw-demo-REPLACEME/backends/aoai-swn?api-version=2024-05-01" `
   ...
   az rest --method PATCH `
     --uri "https://management.azure.com/subscriptions/00000000-0000-0000-0000-000000000000/resourceGroups/rg-lab-aigw-demo-swc/providers/Microsoft.ApiManagement/service/apim-lab-aigw-demo-REPLACEME/backends/aoai-swn?api-version=2024-05-01" `
     --body '{\"properties\":{\"url\":\"https://aoai-lab-aigw-swn-REPLACEME.openai.azure.com/openai\",\"protocol\":\"http\"}}'
   ```
6. (Optional, only if you'll demo §7 sticky and want to peek at Redis directly):
   ```powershell
   az redis list-keys -g rg-lab-aigw-demo-swc -n redis-lab-aigw-REPLACEME --query primaryKey -o tsv
...
**Why this matters for the platform team:**
This is the stable contract every tenant platform tenant calls. From here, every subsequent feature is a *policy*, not a redeployment.
...
**Why this matters for the platform team:**
Replaces the 429-only client-side breaker with a *server-side, deterministic, multi-condition* breaker that every tenant platform tenant inherits — no SDK update across the fleet.
...
**Why this matters for the platform team:**
The same prompt, the same backend, the same model — different SLA contract per tenant key. That's per-tier governance without touching the model layer. Maps cleanly onto your tenant platform tenant tiers (free/pro/enterprise) or your workload tiers (the embedded assistant UI, agentic-batch, interactive-UI).
...
**What you say:**
> "Same prompt twice, two seconds apart. The second call doesn't reach AOAI — APIM hashes the embedding, compares against Redis with `score-threshold=0.5`, returns the cached completion. For your tenant platform-tenant prompts, the hit rate goes up the more identical FAQ-style prompts they send."
...
**Why this matters for the platform team:**
For repetitive tenant platform-tenant prompts (the embedded assistant UI canned questions, agent skill probes, structured-output retries), this is a *direct token-bill saver*. The token-emit metric drops to zero on cache hit — chargeback shows the right thing.
...
**Why this matters for the platform team:**
One enforcement point for the whole tenant platform fleet. You don't rely on every model client to call Content Safety correctly — and you don't trust the model's own moderation as the only line.
...
**Why this matters for the platform team:**
Direct chargeback dimension. Per tenant platform subaccount, per tier, per backend, per minute — exportable to your billing pipeline.
...
> "Three takeaways. (1) Resiliency moves from client SDK to gateway policy — multi-condition CB + active probe + weighted pool. (2) Per-tenant governance — timeout, quota, content safety, attribution — lives one layer below the model and above your callers. (3) Same gateway scales to MCP and to non-AOAI providers without changing the tenant platform-tenant contract.
>
> What we'd refine for production at the platform team scale: APIM Premium for AZ + multi-region active-active gateway, Standard Redis with AOF, JWT validation against your IdP, and Defender for APIs on top. Happy to walk through that as a follow-up."
...
| Section 2 — every call 500, none green | `aoai-swc` broken accidentally; you patched the wrong backend | RESTORE all three: SWN, SWC, FRC URLs (the SWC URL is `https://aoai-lab-aigw-swc-REPLACEME.openai.azure.com/openai`) |
...
3. Confirm `az group show -n rg-lab-aigw-demo-swc` returns 404.
